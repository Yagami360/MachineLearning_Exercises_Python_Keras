...
def main():
    """
    Keras Functional API によるニューラルネットワーク
    """
    print( "Start main()" )
    ...
    #======================================================================
    # Input (Tensor) の定義
    input_tsr = keras.layers.Input( shape = (784,) )

    #network = keras.layers.Dense( units = n_classes, activation = "relu" )
    #print( "network :", network )     # 

    network = keras.layers.Dense( units = n_classes, activation = "relu" )( input_tsr ) # Dense クラスのインスタンスに対し、__call__() を呼び出し。
    print( "network :", network )     # network Tensor("dense_2/Relu:0", shape=(?, 10), dtype=float32)

    network = keras.layers.Dense( units = n_classes, activation = "softmax" )( network )

    # Functional API では、モデルの入出力を設定することで、モデルを構築
    model = keras.models.Model( inputs = input_tsr, outputs = network )
